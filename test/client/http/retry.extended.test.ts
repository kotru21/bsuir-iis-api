...
describe("getRetryDelayMs", () => {
  ...
  it("returns exact baseDelay when retryJitter is false (line 57)", () => {
    const config = makeConfig({ retryJitter: false, retryDelayMs: 300, retryMaxDelayMs: 3000 });
  ...
  it("applies jitter factor in [0.75, 1.25] range when retryJitter is true (line 58)", () => {
    const config = makeConfig({ retryJitter: true, retryDelayMs: 300, retryMaxDelayMs: 3000 });
    for (let i = 0; i < 20; i++) {
      const delay = getRetryDelayMs(config, 0);
  ...
  it("caps baseDelay at retryMaxDelayMs (exponential overflow)", () => {
    const config = makeConfig({ retryJitter: false, retryDelayMs: 300, retryMaxDelayMs: 1000 });
